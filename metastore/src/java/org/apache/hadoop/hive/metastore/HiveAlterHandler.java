...
public class HiveAlterHandler  implements AlterHandler {
  ...
  throws InvalidOperationException, MetaException {
    ...
    try {
      ...
      Partition oldPriT = oldt.getPriPartition();
      Partition newPriT = newt.getPriPartition();
      if (oldPriT != null && newPriT != null) {
	      if (!oldPriT.getParKey().equals(newPriT.getParKey())) {
	        throw new InvalidOperationException("primary partition key can not be changed.");
	      }
      } else {
        if (oldPriT != null || newPriT != null) {
          throw new InvalidOperationException("Do we need to support alter partition in alter table operation?");
        }
      }
      
      Partition oldSubT = oldt.getSubPartition();
      Partition newSubT = newt.getSubPartition();
      
      if (oldSubT != null && newSubT != null) {
        if (!oldSubT.getParKey().equals(newSubT.getParKey())) {
          throw new InvalidOperationException("sub partition keys can not be changed.");
        }
      } else {
        if (oldSubT != null || newSubT != null) {
          throw new InvalidOperationException("Do we need to support alter partition in alter table operation?");
...
//      }
      ...
      if (rename  // if this alter is a rename
        ...
        // Modification By : guosijie
        // Modification Date : 2010-03-10
        //   current we just store a relate path of each partition, so we do not need to change this
