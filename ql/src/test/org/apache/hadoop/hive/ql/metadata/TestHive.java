...
public class TestHive extends TestCase {
  ...
  public void testTable() throws Throwable {
    try {
     ...
     FieldSchema partCol1 = new FieldSchema("ds", Constants.STRING_TYPE_NAME, 
      "partition column, date but in string format as date type is not yet supported in QL");
     
     FieldSchema partCol2 = new FieldSchema("gender", Constants.STRING_TYPE_NAME, 
     "partition column, date but in string format as date type is not yet supported in QL");
  ...
  ArrayList<String> girl = new ArrayList<String>();
      ...
      PartitionDesc SubPartdesc = new PartitionDesc(PartitionType.LIST_PARTITION,partCol2,ps2,null);
      SubPartdesc.setDbName(MetaStoreUtils.DEFAULT_DATABASE_NAME);
      SubPartdesc.setTableName("table_for_testtable");
      

      PartitionDesc PriPartdesc = new PartitionDesc(PartitionType.RANGE_PARTITION,partCol1,ps1,SubPartdesc);
      PriPartdesc.setDbName(MetaStoreUtils.DEFAULT_DATABASE_NAME);
      PriPartdesc.setTableName("table_for_testtable");
      //PartitionDesc partdesc = new PartitionDesc(PartitionType.LIST_PARTITION,partCol, ps,null);
...
//      tbl.setPartCols(partCols);
      ...
      try {
        ...
        assertEquals("the pri partition type RANGE",tbl.getTTable().getPriPartition().getParType().toLowerCase(),ft.getTTable().getPriPartition().getParType().toLowerCase());
        assertEquals("the sub partition type LIST",tbl.getTTable().getSubPartition().getParType().toLowerCase(),ft.getTTable().getSubPartition().getParType().toLowerCase());
        
        assertEquals("the pri partition size",ft.getTTable().getPriPartition().getParSpaces().size(),2);
        
        assertEquals("the sub partition size",ft.getTTable().getSubPartition().getParSpaces().size(),2);
        
        assertEquals("the pri partition p1 less than value size",ft.getTTable().getPriPartition().getParSpaces().get("p1_less10").get(0),"10");
        
        assertEquals("the pri partition p2 less than value size",ft.getTTable().getPriPartition().getParSpaces().get("p2_less20").get(0),"20");
        
        assertEquals("the sub partition sp1 list values index 1",ft.getTTable().getSubPartition().getParSpaces().get("sp1_boy").get(0),"boy");
        
        assertEquals("the sub partition sp2 list values index 3",ft.getTTable().getSubPartition().getParSpaces().get("sp2_girl").get(2),"f");
  ...
  //we test partition in the testTable func
