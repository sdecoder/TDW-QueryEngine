...
//    
//    partdesc.setDbName(MetaStoreUtils.DEFAULT_DATABASE_NAME);
//    
...
//    
    ...
    // create the temp partition table
    // because we do not support "LOAD" into a partition table now.
    // we use "insert ... select * from ..." to do the data loading.
    ...
    FieldSchema subPartKey = new FieldSchema("hr", Constants.STRING_TYPE_NAME, "default");
    ...
    PartitionDesc subPartDesc = new PartitionDesc(MetaStoreUtils.DEFAULT_DATABASE_NAME, "srcpart",
        PartitionType.RANGE_PARTITION, subPartKey, subPartSpaces, null);
    
    // create the primary partition
    FieldSchema priPartKey = new FieldSchema("ds", Constants.STRING_TYPE_NAME, "default");
    String[][] partValues = new String[][] { 
      new String[] { "2008-04-08" },
    ...
    PartitionDesc priPartDesc = new PartitionDesc(MetaStoreUtils.DEFAULT_DATABASE_NAME, "srcpart",
    ...
    // db.createTable("srcpart",cols,null,TextInputFormat.class, IgnoreKeyTextOutputFormat.class);
    db.createTable("srcpart", ptCols, priPartDesc, TextInputFormat.class, IgnoreKeyTextOutputFormat.class);
    ...
    for (String ds: new String[]{"2008-04-08", "2008-04-09"}) {
      for (String hr: new String[]{"11", "12"}) {
        ...
        runLoadCmd("LOAD DATA INPATH '" +  newfpath.toString() +
                   "' OVERWRITE INTO TABLE srcpart");
      }
    }
    **/
    
    // runLoadCmd("INSERT OVERWRITE TABLE srcpart SELECT * FROM temp_srcpart");
  ...
  public void init(String tname) throws Exception {
    ...
    PartitionDesc subPartDesc = new PartitionDesc(MetaStoreUtils.DEFAULT_DATABASE_NAME, "dest3",
        PartitionType.RANGE_PARTITION, subPartKey, subPartSpaces, null);
    ...
    PartitionDesc priPartDesc = new PartitionDesc(MetaStoreUtils.DEFAULT_DATABASE_NAME, "dest3",
        PartitionType.LIST_PARTITION, priPartKey, priPartSpaces, subPartDesc);
    
    db.createTable("dest3", part_cols, priPartDesc, TextInputFormat.class, IgnoreKeyTextOutputFormat.class);


    /** removed by guosijie
    db.createTable("dest3", cols, null, TextInputFormat.class, IgnoreKeyTextOutputFormat.class);
  ...
  public void loadPartition(Path partFile, String tableName, String priPartName, String subPartName) throws IOException {
    StringBuilder sb = new StringBuilder();
    //add by allison-----now we hive DB,so we set db before tablename
    sb.append(MetaStoreUtils.DEFAULT_DATABASE_NAME).append("/").append(tableName).append("/").append(priPartName);
  ...
  public int checkNegativeResults(String tname, Exception e) throws Exception {
    ...
    String cmdLine = "diff -I \\(query:\\) " + outf.getPath() + " " + expf.getPath();
  ...
  public int checkCliDriverResults(String tname) throws Exception {
    ...
    //cmdArray[3] = "\\(file:\\)\\|\\(/tmp/.*\\)\\|\\(query:.*\\)\\|\\(Output:.*\\)";

    cmdArray[3] = "\\(file:\\)\\|\\(/tmp/.*\\)\\|\\(query:\\)\\|\\(Input:\\)\\|\\(Output:\\)\\|\\(compress\\)";
    cmdArray[3] = "\\(:\\)\\|\\(/tmp/.*\\)\\|\\(compress\\)";
  ...
  public List<Task<? extends Serializable>> analyzeAST(ASTNode ast) throws Exception {
	  ...
	  //TODO:Context now has new ctor,we should test the new ctor
