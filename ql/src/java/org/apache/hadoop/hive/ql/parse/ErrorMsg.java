...
public enum ErrorMsg {
  ...
  PARTITIONING_COLS_SHOULD_IN_COLUMNS("partitioning columns should in table columns"),
  ...
  NO_LIMIT_WITH_ORDERBY_FORTOP("Limit must be specified if ORDER BY is present"),//add by bryanxu for top
  INVALID_LIMIT_COUNT_FOR_ORDERBY("Limit value is Too BIG for ORDER BY(Default Max Limit:1024)"),//add by bryanxu for top
  ...
  PROJECTION_TOO_MANY("projection must no exceed 20"),
  PROJECTION_FIELD_DUP("projection field duplicate"),
  PROJECTION_FIELD_NOT_EXIST("projection field not exist"),
  PROJECTION_NOT_DEFINE("field exceed 20, must define projection"),
  
  INDEX_FIELD_DUP("index field duplicate"),
  INDEX_FIELD_NOT_EXIST("index field not exist");
