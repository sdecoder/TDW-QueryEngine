...
public class ASTPartitionPruner {
  ...
  public ASTPartitionPruner(String tableAlias, QB qb/*QBMetaData metaData*/, HiveConf conf) {
    this.qb = qb;
  ...
  throws SemanticException {
    ...
    switch (tokType) {
      case HiveParser.TOK_TABLE_OR_COL: {
        String tableOrCol = BaseSemanticAnalyzer.unescapeIdentifier(expr.getChild(0).getText()).toLowerCase();
      ...
      case HiveParser.DOT: {
        ...
        if (left.getType() == HiveParser.TOK_TABLE_OR_COL) {
          ...
          if (metaData.getAliasToTable().get(fullAlias.toLowerCase()) != null) {
            ...
            return tableAlias.equalsIgnoreCase(fullAlias) && tab.isPartitionKey(colName);
          }
        }
