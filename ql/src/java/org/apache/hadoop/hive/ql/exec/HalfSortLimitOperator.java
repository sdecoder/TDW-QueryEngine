...
public class HalfSortLimitOperator extends Operator<halfSortLimitDesc> implements Serializable {

	static final private Log LOG = LogFactory.getLog(HalfSortLimitOperator.class.getName());
	...
	class RowWrapper {
		...
		public RowWrapper(Object row, ArrayList<ExprNodeEvaluator> keyFields, ArrayList<ObjectInspector> keyInspectors) {
			this.row = row;
		...
		public int compare(Object row1, Object row2) {
			try {
				...
				for (int i = 0; i < keyCount; i++) {
					...
					int r = ObjectInspectorUtils.compare(key1, r1.getKeyInspector(i), key2, r2.getKeyInspector(i));
					if (r != 0) {
						if (orders[i] == false) {
							return -r;
						} else {
	...
	protected void initializeOp(Configuration hconf) throws HiveException {
		try {
			...
			outputObjInspector = ObjectInspectorUtils.getStandardObjectInspector(rowInspector, ObjectInspectorCopyOption.DEFAULT);
			...
			for (int i = 0; i < conf.getSortCols().size(); i++) {
				originKeyFields.add(ExprNodeEvaluatorFactory.get(conf.getSortCols().get(i)));
				originKeyInspectors.add(originKeyFields.get(i).initialize(rowInspector));
				newKeyFields.add(ExprNodeEvaluatorFactory.get(conf.getSortCols().get(i)));
				newKeyInspectors.add(newKeyFields.get(i).initialize(outputObjInspector));
			}
	...
	public void process(Object row, int tag) throws HiveException {
		if (heap.size() < conf.getLimit()) {
			...
			RowWrapper thisRow = new RowWrapper(row, originKeyFields, originKeyInspectors);
		...
		// In case of a lot of rows skipped, send a heartbeat in order to avoid timeout
