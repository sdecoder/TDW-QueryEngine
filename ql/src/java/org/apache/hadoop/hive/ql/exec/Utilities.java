...
import org.apache.hadoop.hive.serde.Constants;  //Added by Brantzhang for patch HIVE-1158
...
public class Utilities {
    ...
    try {
      ...
      if(fs.exists(planPath)) {
           try {
             ...
             LOG.debug("delete Plan ... " + planPath.toUri());
           } catch (IOException e) {
             e.printStackTrace();
           }
      }
    ...
    try {
      if(gWork == null) {
        synchronized (Utilities.class) {
          ...
          LOG.debug("getMapRedWork : getFD " + ((FileInputStream)in).getFD().toString());
  ...
  public static void setMapRedWork (Configuration job, mapredWork w) {
    try {
      ...
      System.out.println("setMapRedWord : planPath :" + planPath.toUri());
      
      FSDataOutputStream out = fs.create(planPath);
      serializeMapRedWork(w, out);
      
      //allison :for debug ,we save a copy of the plan to the local
      Path tmpPath=new Path("file:///tmp/hive-plan" + randGen.nextInt());
      LOG.debug("setMapRedWord : tmpPath :" + tmpPath.toUri());
      fs.copyToLocalFile(planPath, tmpPath);
      // end
      ...
      LOG.debug("uriWithLink : " + uriWithLink);
  ...
  public static String getTaskId(Configuration hconf) {
    ...
    //   we add time in the trail of the file name to identify different insert operation
   ...
   */
  static Pattern fileNameTaskIdRegexrow = Pattern.compile("^.*_([0-9]*)_[0-9](\\..*)?$");
  
  // 列存储的文件名可能为:
  // 1. attempt_201008180953_0227_r_000000_0.1282790119492_idx2
  // 2. attempt_201008180953_0227_r_000000_0.1282790119492_idx2_4_6; (可能有多个字段.)
  static Pattern fileNameTaskIdRegexcolumn = Pattern.compile("^.*_([0-9]*)_[0-9,.]*_idx([0-9]*)[\\w]*(\\..*)?$");
  ...
  // joeyli changed for column store begin 
  public static String getTaskIdFromFilename(String filename) {
    ...
    if (!mrow.matches()&&!mcolumn.matches()) {
      ...
      if(mcolumn.matches())
      {
        ...
        LOG.info("TaskId for " + filename + " = " + taskId);
        return taskId;
      }      
  ...
  // joeyli changed for column store end 
  /**
