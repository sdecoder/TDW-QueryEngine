...
public class Driver implements CommandProcessor {
  ...
  public Schema getSchema() throws Exception {
    ...
    try {
      if (plan != null && plan.getPlan().getFetchTask() != null) {
        ...
        //TDW' fetchWork don't have partition info! so blow is not used
  ...
  public Driver(HiveConf conf) {
    this.conf = conf;
    getSessionState();//add by allison,for some test direct call driver and not create sessionstate
    ctx = new Context (conf,SessionState.get().getUserName(),SessionState.get().getDbName());
  }
  public Driver() {
    if (SessionState.get() != null) {
      conf = SessionState.get().getConf();
  ...
  public int run(String command) {
    ...
    if(SessionState.get() != null && SessionState.get().isCheck()){
    	LOG.info("check SQL semantic OK!");
    	return 0;
    }
    return execute();
  }
