...
public class HiveMetaStoreChecker {
  ...
  public void checkMetastore(String dbName, String tableName,
      QB.tableRef trf, CheckResult result) 
    ...
    try {
      ...
      } else if (trf == null || trf.getPriPart() == null && trf.getSubPart() == null) {
  ...
  void checkTable(String dbName, String tableName,
    ...
    //List<org.apache.hadoop.hive.metastore.api.Partition> parts = new ArrayList<org.apache.hadoop.hive.metastore.api.Partition>();
    ...
    if (table.isPartitioned()) {
      if (trf == null || trf.getPriPart() == null && trf.getSubPart() == null) {
        ...
        //parts = hive.getPartitions(table);
    	  ...
    	  parts = Warehouse.getPartitionPaths(table.getPath(),table.getTTable().getPriPartition().getParSpaces().keySet(), (table.getTTable().getSubPartition() !=null ? table.getTTable().getSubPartition().getParSpaces().keySet() : null));
      } else {
        //we're interested in specific partitions,
        //don't check for any others
        findUnknownPartitions = false;//user specific partition,so this is false
       
        if(trf.getPriPart() != null){
        	if(table.getTTable().getPriPartition().getParSpaces().containsKey(trf.getPriPart())){
        			parts = Warehouse.getPriPartitionPaths(table.getPath(), trf.getPriPart(), (table.getTTable().getSubPartition() !=null ? table.getTTable().getSubPartition().getParSpaces().keySet() : null));
        	...
        	if(table.getTTable().getSubPartition() != null && table.getTTable().getSubPartition().getParSpaces().containsKey(trf.getSubPart())){
        		parts = Warehouse.getSubPartitionPaths(table.getPath(), table.getTTable().getPriPartition().getParSpaces().keySet(), trf.getSubPart());
...
//        for (org.apache.hadoop.hive.metastore.api.Partition pt : partitions) {
...
//        }
   ...
   */
  void checkTable(Table table, List<Path> parts, 
      boolean findUnknownPartitions, CheckResult result,String partName,boolean isSub) 
