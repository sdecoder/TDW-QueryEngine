...
//import java.io.ObjectInputStream;  //Removed by Brantzhang for patch HIVE-1137
//import java.io.ObjectOutputStream; //Removed by Brantzhang for patch HIVE-1137
import java.io.DataInputStream;      //Added by Brantzhang for patch HIVE-1137
import java.io.DataOutputStream;     //Added by Brantzhang for patch HIVE-1137
...
import org.apache.hadoop.hive.ql.plan.tableDesc;  //Added by Brantzhang for patch HIVE-1158 
...
public class RowContainer<Row extends List> {
  ...
  //private static final int BLKMETA_LEN = 100; // default # of block metadata: (offset,length) pair  //Removed by Brantzhang to save memory
  private static final int BLKMETA_LEN = 1; // default # of block metadata: (offset,length) pair  //Added by Brantzhang to save memory
  ...
  private ArrayList dummyRow; // representing empty row (no columns since value art is null) //Added by Brantzhang for patch HIVE-1037
  private tableDesc tblDesc;   //Added by Brantzhang  for patch HIVE-1158
  ...
  public RowContainer(int blockSize) {
    ...
    //this.off_len   = new long[BLKMETA_LEN * 2];  //Removed by Brantzhang to save memory
    this.serde     = null;
    this.standardOI= null;
    this.dummyRow  = new ArrayList(0);   //Added by Brantzhang for patch HIVE-1158
  }
  ...
  private void spillBlock(Row[] block) throws HiveException {
    try {
      if ( tmpFile == null ) {
        ...
        off_len   = new long[BLKMETA_LEN * 2]; //Added by Brantzhang to save memory
      }
      byte[] buf = serialize(block);
      //long offset = rFile.getFilePointer();  //Removed by Brantzhang for patch HIVE-1037
      long offset = rFile.length();  //Added by Brantzhang for patch HIVE-1037
      long len = buf.length;
  ...
  private byte[] serialize(Row[] obj) throws HiveException {
    ...
    //ObjectOutputStream     oos;  //Removed by Brantzhang for patch HIVE-1037
    DataOutputStream     oos;  //Added by Brantzhang for patch HIVE-1137
    
    try {
      baos = new ByteArrayOutputStream();
      //oos = new ObjectOutputStream(baos);  //Removed by Brantzhang for patch HIVE-1037
      oos = new DataOutputStream(baos);  //Added by Brantzhang for patch HIVE-1037
      
      // # of rows
      oos.writeInt(obj.length); 
      
      //for ( int i = 0; i < obj.length; ++i ) {  //Removed by Brantzhang for patch HIVE-1037
        //Writable outVal = serde.serialize(obj[i], standardOI);  //Removed by Brantzhang for patch HIVE-1037
        //outVal.write(oos);  //Removed by Brantzhang for patch HIVE-1037
  ...
  private Row[] deserialize(byte[] buf) throws HiveException {
    ...
    //ObjectInputStream     ois; //Removed by Brantzhang for patch HIVE-1037
    DataInputStream     ois;     //Added by Brantzhang for patch HIVE-1037
    
    try {
      bais = new ByteArrayInputStream(buf);
      //ois = new ObjectInputStream(bais); //Removed by Brantzhang for patch HIVE-1037
      ois = new DataInputStream(bais);     //Added by Brantzhang for patch HIVE-1037
      int sz = ois.readInt();
      assert sz == blockSize: 
             "deserialized size " + sz + " is not the same as block size " + blockSize;
      Row[] ret = (Row[]) new ArrayList[sz];
      
      // if serde or OI is null, meaning the join value is null, we don't need  //Added by Brantzhang for patch HIVE-1037
      // to serialize anything to disk, just need to keep the length.           //Added by Brantzhang for patch HIVE-1037
      for ( int i = 0; i < sz; ++i ) {
    	if ( serde != null && standardOI != null ) {  //Added by Brantzhang for patch HIVE-1037
    	  Writable val = serde.getSerializedClass().newInstance();  //Added by Brantzhang for patch HIVE-1037
    	  val.readFields(ois);  //Added by Brantzhang for patch HIVE-1037
        ...
        ret[i] = (Row) ObjectInspectorUtils.copyToStandardObject(
           ...
           */
        //Removed by Brantzhang for patch HIVE-1037 End
    	//Added by Brantzhang for patch HIVE-1037 Begin
    	  ret[i] = (Row) ObjectInspectorUtils.copyToStandardObject(
    			            serde.deserialize(val),
    			            serde.getObjectInspector(),
  ...
  public void clear() throws HiveException {
    ...
    addCursor = 0;
    numBlocks = 0;
    //size = 0;   //Removed by Brantzhang for patch HIVE-1037
    pBlock    = 0;  //Added by Brantzhang for patch HIVE-1037
    size      = 0;  //Added by Brantzhang for patch HIVE-1037
    try {
      if ( rFile != null )
        rFile.close();
      if ( tmpFile != null )
