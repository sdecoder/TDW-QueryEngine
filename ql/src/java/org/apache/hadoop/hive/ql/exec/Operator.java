...
import java.util.HashSet;     //Added by Brantzhang for patch HIVE-870
...
public abstract class Operator <T extends Serializable> implements Serializable, Node {
  ...
  public String dump(int level, HashSet<Integer> seenOpts) {
    ...
    if (childOperators != null) {
      ...
      s.append("  <Children>");
      for (Operator<? extends Serializable> o : childOperators) {
        //s.append(o.dump(level+2));            //Removed by Brantzhang for patch HIVE-870
    	s.append(o.dump(level+2, seenOpts));    //Added by Brantzhang for patch HIVE-870
      }
      s.append(ls);
      s.append("  <\\Children>");
    }
    ...
    if (parentOperators != null) {
      ...
      for (Operator<? extends Serializable> o : parentOperators) {
        s.append("Id = " + o.id + " ");
        s.append(o.dump(level,seenOpts));       //Added by Brantzhang for patch HIVE-870
      }
      s.append("<\\Parent>");
    }
