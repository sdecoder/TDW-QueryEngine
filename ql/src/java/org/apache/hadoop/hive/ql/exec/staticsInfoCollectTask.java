 ...
 **/
public class staticsInfoCollectTask extends Task<staticsInfoCollectWork> implements Serializable {
...
public tdw_sys_fields_statistics getFields_statistic(String field)
{
  ...
  public int execute() {
	...
	{
		 LOG.error("staticsInfoCollectTask Err:"+dir.toString()+" is not a directory");
		 console.printError("staticsInfoCollectTask Err:"+dir.toString()+" is not a directory");
		 return -1;
	}
	FileStatus[] fss=fs.listStatus(dir);
	if(fss.length!=1)
	{
		 LOG.error("staticsInfoCollectTask Err:"+dir.toString()+" must have only one file");
		 console.printError("staticsInfoCollectTask Err:"+dir.toString()+" must have only one file");
		 return -1;
	}
	FileStatus f=fss[0];
	...
	{
		LOG.error("staticsInfoCollectTask Err:"+f.getPath()+" is a directory");
		return -1;
	}
	 ...
	 for(tdw_sys_fields_statistics fields_statistic: fields_statistics.values())
	 {
		 Hive.get().delete_fields_statistics(fields_statistic.getStat_table_name(), fields_statistic.getStat_field_name());
		 Hive.get().add_fields_statistics(fields_statistic);
	 }     
    
	 // 打印调试信息
	 tdw_sys_table_statistics table=Hive.get().get_table_statistics(table_statistics.getStat_table_name());
	 LOG.error(table.toString());
     Hive.mapjoinstat canMapJoin=Hive.get().canTableMapJoin(table.getStat_table_name());
     LOG.error(table.getStat_table_name()+" canMapJoin="+canMapJoin);

	 
	 List<tdw_sys_fields_statistics> fields=Hive.get().get_fields_statistics_multi(table.getStat_table_name(),-1);
	 for(tdw_sys_fields_statistics field :fields)
	 {
		 ...
		 LOG.error(field.getStat_table_name()+"."+field.getStat_field_name()+" skewstat="+st);
	 }	 
  ...
  private int processLine(String Line,boolean bFirst) {
	    ...
	    if(Line.contains(stat_num_records_token))
	    {
	    	...
	    	//console.printInfo("table_statistics.Stat_num_records="+table_statistics.getStat_num_records());
	    	return 0;
	    }
	    ...
	    if(Line.contains(stat_num_units_token))
	    {
	    	...
	    	//console.printInfo("table_statistics.stat_num_units="+table_statistics.getStat_num_units());
	    	return 0;
	    }
	    ...
	    if(Line.contains(stat_total_size_token))
	    {
	    	...
	    	//console.printInfo("table_statistics.stat_total_size="+table_statistics.getStat_total_size());
	    	return 0;
	    }
	    ...
	    if(Line.contains(stat_num_files_token))
	    {
	    	...
	    	//console.printInfo("table_statistics.stat_num_files="+table_statistics.getStat_num_files());
	    	return 0;
	    }	    
	    ...
	    if(Line.contains(stat_num_blocks_token))
	    {
	    	...
	    	//console.printInfo("table_statistics.stat_num_blocks="+table_statistics.getStat_num_blocks());
	    	return 0;
	    }	
	    ...
	    if(Line.contains(stat_nullfac_token))
	    {
		    ...
		    if(strs.length!=4)
		    {
		    	 //LOG.error("stat_nullfac_token err strs.length!=4"+Line);
		    	 //console.printInfo("stat_nullfac_token err strs.length!=4"+Line);
		    	 return -1;
		    }
		    else
		    {
	    ...
	    if(Line.contains(stat_avg_field_width_token))
	    {
		    ...
		    if(strs.length!=4)
		    {
		    	 //LOG.error("stat_avg_field_width_token err strs.length!=4"+Line);
		    	 //console.printInfo("stat_avg_field_width_token err strs.length!=4"+Line);
		    	 return -1;
		    }
		    else
		    {
		    	String field=strs[2].trim();
		    	String number=strs[3].trim();
		    	getFields_statistic(field).setStat_avg_field_width(Integer.valueOf(number));
		  	    return 0;
		    }
	    }	
	    ...
	    if(Line.contains(stat_distinct_values_token))
	    {
		    ...
		    if(strs.length!=4)
		    {
		    	 //LOG.error("stat_distinct_values_token err strs.length!=4"+Line);
		    	 //console.printInfo("stat_distinct_values_token err strs.length!=4"+Line);
		    	 return -1;
		    }
		    else
		    {
		    	String field=strs[2].trim();
		    	String number=strs[3].trim();
		    	getFields_statistic(field).setStat_distinct_values(Double.valueOf(number));
		  	    return 0;
		    }
	    }		    
	    ...
	    if(Line.contains(Mcvlist_field_token))
	    {
		    ...
		    if(MCVListProcessing)
		    {
			    ...
			    if(strs.length!=6)
			    {
			    	 //LOG.error("MCVListProcessing err strs.length!=6"+Line);
			    	 //console.printInfo("MCVListProcessing err strs.length!=6"+Line);
			    	 return -1;
			    }
			    else
			    {
		    ...
		    if(HistogramTableProcessing)
		    {
			    ...
			    if(strs.length!=5)
			    {
			    	 //LOG.error("HistogramTableProcessing err strs.length!=5"+Line);
			    	 //console.printInfo("HistogramTableProcessing err strs.length!=5"+Line);
			    	 return -1;
			    }
			    else
			    {
