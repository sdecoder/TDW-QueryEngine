...
public class LoadSemanticAnalyzer extends BaseSemanticAnalyzer {
  ...
  public void analyzeInternal(ASTNode ast) throws SemanticException {
    ...
    try {
		if(db.getTable(SessionState.get().getDbName(), ts.tableName).isPartitioned() || ts.partSpec != null){
			throw new SemanticException("TDW do not support load data to partitioned table directly,use insert ....select method instead!");
		}
		
		if(!db.hasAuth(SessionState.get().getUserName(), Hive.Privilege.INSERT_PRIV, SessionState.get().getDbName(), ts.tableName)){
			throw new SemanticException("user : " + SessionState.get().getUserName() + " do not have insert privilege on table :" + SessionState.get().getDbName() + "." + ts.tableName );
		}
		
		if(isOverWrite && !db.hasAuth(SessionState.get().getUserName(), Hive.Privilege.INSERT_PRIV, SessionState.get().getDbName(), ts.tableName)){
			throw new SemanticException("user : " + SessionState.get().getUserName() + " do not have delete(or overwrite) privilege on table :" + SessionState.get().getDbName() + "." + ts.tableName );
		}
    ...
    List<FieldSchema> parts = null;//TODO more thinking ts.tableHandle.getTTable().getPartitionKeys();
