      ...
      if(setDb){//db::tab
          ...
          if(null != ctx.getQb().getUserAliasFromDBTB(fullAlias)){
        	  if(ctx.getQb().getUserAliasFromDBTB(fullAlias).size() > 1){
          		ctx.setError(ErrorMsg.INVALID_TABLE_OR_COLUMN.getMsg(expr.getChild(0).getChild(0)));
					LOG.debug("table : " + fullAlias + "  has more than one alias : " + ctx.getQb().getUserAliasFromDBTB(fullAlias).get(0) + " and " + ctx.getQb().getUserAliasFromDBTB(fullAlias).get(0) + " ......");
					return null;
          	}
        	  fullAlias = fullAlias + "#" + ctx.getQb().getUserAliasFromDBTB(fullAlias).get(0);
          }
    	  ...
    	  if(ctx.getQb().existsUserAlias(tableOrCol)){
    		  ...
    		  if(ctx.getQb().getSubqAliases().contains(tableOrCol.toLowerCase()) && input.hasTableAlias(tableOrCol.toLowerCase())){//subAlias
    			  ...
    			  LOG.debug("db name : " + ctx.getQb().getTableRefFromUserAlias(tableOrCol).getDbName());
    			  LOG.debug("tble name : " + ctx.getQb().getTableRefFromUserAlias(tableOrCol).getTblName());
    			  
    			  fullAlias = ctx.getQb().getTableRefFromUserAlias(tableOrCol).getDbName() + "/" + ctx.getQb().getTableRefFromUserAlias(tableOrCol).getTblName() + "#" + tableOrCol;
    			  LOG.debug("a user set alias....fullAlias is : " + fullAlias);
    			  if(ctx.getQb().getTabAliases().contains(fullAlias.toLowerCase()) &&  input.hasTableAlias(fullAlias.toLowerCase())){
    				  ;
    ...
    public Object process(Node nd, Stack<Node> stack, NodeProcessorCtx procCtx,
      ...
      if (expr.getType() == HiveParser.DOT
        ...
        if(expr.getChild(0).getChildCount() == 2){
        	db = SemanticAnalyzer.unescapeIdentifier(
                    expr.getChild(0).getChild(1).getText());
        	isSetDb = true;
        }else{
        	db = SessionState.get().getDbName();
        }
        ...
        if(isSetDb){//db::tab
      	  ...
      	  if(ctx.getQb().existsUserAlias(tableAlias)){
      		  if(ctx.getQb().getSubqAliases().contains(tableAlias.toLowerCase()) && input.hasTableAlias(tableAlias)){//subAlias
      			  fullAlias = tableAlias;
      		  }else{//user table alias
      			  //user table alias and sub alias will uniq
      			  fullAlias = ctx.getQb().getTableRefFromUserAlias(tableAlias).getDbName() + "/" + ctx.getQb().getTableRefFromUserAlias(tableAlias).getTblName()+ "#" + tableAlias;
      			  if(ctx.getQb().getTabAliases().contains(fullAlias.toLowerCase()) &&  input.hasTableAlias(fullAlias)){
      				  ;
      			  }else{
      				  fullAlias = null;
      			  }
      		  ...
      		  //it is a table name?
      		  if(ctx.getQb().exisitsDBTB(db + "/" + tableAlias) /*&& input.hasTableAlias(db + "/" + tableAlias)*/){
      			  if(fullAlias != null){
      				  if(!(db + "/" + tableAlias + "#" + tableAlias).equalsIgnoreCase(fullAlias)){
      					  ctx.setError(ErrorMsg.INVALID_TABLE_OR_COLUMN.getMsg(expr.getChild(0).getChild(0)));
      					  LOG.debug("TDW can't jude the : " + tableAlias + " in current session! " + ErrorMsg.INVALID_TABLE_OR_COLUMN.getMsg(expr.getChild(0).getChild(0)));
      					  return null;
      				  }else
      					  ;
      			  }
        ...
        if (colInfo == null) {
          ...
          LOG.debug("DefaultExprProcessor : " + ErrorMsg.INVALID_COLUMN.getMsg(expr.getChild(1)));
          return null;
        }
        return new exprNodeColumnDesc(colInfo.getType(), colInfo.getInternalName(),
                                      colInfo.getTabAlias(), colInfo.getIsPartitionCol());
