...
//  }
  ...
  private void getNextPath() throws Exception {
    // first time
	  ...
	  //now tdw do not allow data be store in table dir if the table partitioned
	  
    if (iterPath == null) {//如果该路径迭代器尚未设置，则从fetchWork中读取
//      if (work.getTblDir() != null) {//get table dir,not partitioned table
//        if (!tblDataDone) {
...
//          }
//
//          if (!tblDataDone)//all file is empty! so current path is null
//            currPath = null;
//          return;
//        } 
//        else {//this path has bean done!
//          currTbl = null;
//          currPath = null;
//        }
//        return;
//      }//above deal with the table without partition 
//      else {//with partition
//        
//    	  throw new Exception("fetch op without table dir");
//    	  
...
//      }
    	...
    	currTbl = work.getTblDesc();
    	iterPath = fetchWork.convertStringToPathArray(work.getFetchDir()).iterator();
    }
...
//        if (tblDataDone) {
//        	if(serde == null)
//        		throw new Exception("serde is null pointer");
