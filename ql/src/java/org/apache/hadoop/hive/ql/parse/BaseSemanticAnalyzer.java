...
public abstract class BaseSemanticAnalyzer {
  ...
  protected int numOfHashPar; //Added by Brantzhang for hash partition
  ...
  public static class tableSpec {
    ...
    public tableSpec(Hive db, HiveConf conf, ASTNode ast) throws SemanticException {
      ...
      try {
        // get table metadata获取表的元数据
        tableName = unescapeIdentifier(ast.getChild(0).getText());
        
        if(ast.getChildCount() == 2){
        	int token = ((ASTNode)(ast.getChild(1))).getToken().getType();
        	if(token != HiveParser.TOK_PARTITIONREF 
        		&& token !=HiveParser.TOK_SUBPARTITIONREF && token != HiveParser.TOK_COMPPARTITIONREF){
        		dbName = unescapeIdentifier(ast.getChild(1).getText());
        	
        	}else
        		throw new SemanticException("TDW do not allow to insert into partition directly!");
        }
        ...
        try{
        db.getDatabase(dbName);//test if db exists
        }catch (Exception e){
      	  throw new SemanticException("get database error : " + dbName + " ,make sure it exists!");
        }
        
        tableHandle = db.getTable(dbName, tableName);//获取表的元数据
      ...
      // get partition metadata if partition specified如果指明了分区，则获取分区的元数据
//      if (ast.getChildCount() == 2) {//两个子节点表明指定了分区
//        childIndex = 1;
