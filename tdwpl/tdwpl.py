...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "hd:f:a:e:ls:u:x:y:j:v:p:", 
    ...
    try:
            ...
            if action == None or fname == None:
                ...
                sys.path.append("%s/lib/%s" 
                                % ("~/.tdwpl/".replace("~", 
                                                       os.getenv("HOME")),
                                   user))
                module = __import__(os.path.basename(fname).rstrip(".py"))
        else:
        ...
        if action == "del":
            ...
            for idx in range(0, len(exec_string)):
                if exec_string[idx] != "":
                    res = client.execute(exec_string[idx])
                    if res == "success":
                        res = client.fetchAll()
                    print "Execute '%s' replied ok w/ : " % (exec_string[idx])
                    printlist(res)
        elif action == "execf":
            debug = 1
            try:
